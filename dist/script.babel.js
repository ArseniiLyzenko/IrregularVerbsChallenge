...
var span = '<span id="carriage">&nbsp;</span>'; // The &nbsp (non-breaking space or just whitespace) need to fix unexpected behavior of span tag

var couldNotLoad = '[["1","null","null","null","Could not load the table of irregular verbs :("]]';
var irregularVerbsJsonFile = "verbs100array.json";
var gameOver = "Game Over";
var wellPlayed = "Well Played!";
var typingSpeed = 100;
var untypingSpeed = 70; // how to make constant-global variables and declare them after page loading?!
...
var sectionThree_section; //var scoreZeroPopup_span;

var scoreTable_div; //var score_table;
...
function onLoad() {
  ...
  sectionThree_section = document.getElementById('section-three_section'); //scoreZeroPopup_span   = document.getElementById("score-zero-popup_span");

  scoreTable_div = document.getElementById('score-table_div'); //score_table       = document.getElementById('score_table');
...
var newVerbsOnly = function (verbID) {
  ...
  return {
    get: function get() {
      return _newVerbsOnly;
    },
    reSet: function reSet() {
      _newVerbsOnly = verbs.get().slice();
      console.log("newVerbsOnly.reSet();");
    },
    length: function length() {
      return _newVerbsOnly.length;
    },
    splice: function splice(verbID, param) {
      _newVerbsOnly.splice(verbID, param);

      return;
    },
    setVerb: function setVerb(verbID) {
      this.pickVerb(this.randVerbID());
      console.log("picked_verb_id: " + verb.get()[0] + " " + verb.get()[1] + " " + verb.get()[2] + " " + verb.get()[3]);
      setTimeout(function () {
        typingAnimation(rusVerb_p, verb.get()[4], typingSpeed);
      }, (rusVerb_p.innerHTML.length - span.length) * 100);
    },
    pickVerb: function pickVerb(verbID) {
      var newVerb = newVerbsOnly.get(); //---------------------------!-----------------

      verb.set(newVerbsOnly.get()[verbID]);
      newVerbsOnly.splice(verbID, 1);
    },
    randVerbID: function randVerbID() {
...
function bringItOn() {
  ...
  if (isTypingSpaceClear(rusVerb_p)) {
    ...
    }, 1500); //wait for scrolling

    setTimeout(function () {
      newVerbsOnly.setVerb();
    }, 1500); //wait for untyping animation
  } //setTimeout(function () {scrollToGameAreaSector()}, 1000);


  sectionOne_section.style.display = "none"; //setTimeout(function () {sectionOne_section.style.display = "none"}, 700);
...
function win() {
  ...
  newVerbsOnly.reSet(); // v1_input.disabled           = true;
  ...
  scrollToGameOverSector(); //setTimeout(function () {sectionTwo_section.style.display = "none"}, 700);

  sectionTwo_section.style.display = "none"; //setTimeout(function () {name_input.focus()}, 700);
}
...
function backToIntro() {
  score.reset();
  scrollToIntroSector(); //setTimeout(function () {sectionThree_section.style.display = "none"}, 700);

  sectionThree_section.style.display = "none"; // bringItOn_btn.disabled      = false;
...
function scrollToIntroSector(callback) {
  ...
  sectionOne_section.style.display = "flex"; //sectionOne_section.scrollIntoView({behavior: "smooth"});
}

function scrollToGameAreaSector(callback) {
  console.log("scrollToGameAreaSector()");
  sectionTwo_section.style.display = "flex"; //sectionTwo_section.scrollIntoView({behavior: "smooth"});
}

function scrollToGameOverSector(callback) {
  console.log("scrollToGameOverSector()");
  sectionThree_section.style.display = "flex"; //sectionThree_section.scrollIntoView({behavior: "smooth"});
...
function xmlhttp_() {
  ...
  function buildScoreTable() {
    ...
    for (iterator in resultQuery_obj) {
      ...
      if (names.find(checkDuplicateNames)) {
        continue;
      }

      console.log("resultQuery_obj[iterator].date = " + resultQuery_obj[iterator].date);
      date = new Date(resultQuery_obj[iterator].date);
      table += "<tr>" + "<td>" + ++rowNumScoreTable + "</td>" + "<td>" + resultQuery_obj[iterator].name + "</td>" + "<td>" + resultQuery_obj[iterator].score + "</td>" + "<td>" + formatDate(date) + "</td>" + "</tr>";
